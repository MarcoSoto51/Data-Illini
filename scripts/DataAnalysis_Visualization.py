...
Research Questions:
  ...
  2. How do fatal crash patterns vary across geographic regions, time of day,
     and road types?
  3. What is the impact of seat belt use, alcohol, and other impairments on
     fatality outcomes?
...
Outputs (saved to ./outputs/):
    ...
    figures/10_state_choropleth.png
...
DATA_PATH = "./data/processed/fars_integrated_crash_level.csv"
OUTPUT_DIR = "./outputs"
FIGURES_DIR = os.path.join(OUTPUT_DIR, "figures")
RESULTS_DIR = os.path.join(OUTPUT_DIR, "results")
 
# Codebook mappings (from FARS documentation)
...
def plot_fatals_distribution(df: pd.DataFrame):
    ...
    ax2 = axes[1]
    ru_fat = df[df["ru_label"].isin(["Rural", "Urban"])].groupby("ru_label")["FATALS"].value_counts().unstack(fill_value=0)
...
def plot_geographic(df: pd.DataFrame):
    ...
    ax.barh(reg.index, reg.values, color=colors)
    ax.set_title("Total Crashes by Census Region")
    ax.set_xlabel("Number of Fatal Crashes")
    for i, v in enumerate(reg.values):
        ax.text(v + 50, i, f"{v:,}", va="center", fontsize=8)
 
    ax2 = axes[1]
    state_cnt = df.groupby("state_abbr")["ST_CASE"].count().sort_values(ascending=False).head(15)
...
def plot_time_patterns(df: pd.DataFrame):
    ...
    ax2 = axes[1]
    # Check what values actually exist in A_DOW
    print("A_DOW value counts:", df["A_DOW"].value_counts().sort_index())
    
    dow_counts = df["A_DOW"].value_counts().sort_index()
    dow_order_map = {1: "Sun", 2: "Mon", 3: "Tue", 4: "Wed", 5: "Thu", 6: "Fri", 7: "Sat"}
    dow_counts.index = [dow_order_map.get(i, str(i)) for i in dow_counts.index]
    
    # Reorder Sun–Sat
    desired_order = ["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"]
    dow_counts = dow_counts.reindex([d for d in desired_order if d in dow_counts.index])
    
    bar_colors = [ACCENT if d in ["Fri", "Sat", "Sun"] else "#90CAF9" for d in dow_counts.index]
    ax2.bar(dow_counts.index, dow_counts.values, color=bar_colors)
    ax2.set_title("Crashes by Day of Week")
    ax2.set_xlabel("Day")
    ax2.set_ylabel("Number of Fatal Crashes")
...
def plot_road_environment(df: pd.DataFrame):
    ...
    ax = axes[0]
    road = df["roadfc_label"].value_counts().sort_values(ascending=True)
    ...
    # Weather (exclude unknown/99)
    ax2 = axes[1]
    weather = df[df["A_WEATHER"] != 99]["weather_label"].value_counts().sort_values(ascending=True)
...
def plot_impairment(df: pd.DataFrame):
    ...
    rates = {label: df[col].mean() * 100 for col, label in risk_cols.items() if col in df.columns}
    rates_s = pd.Series(rates).sort_values(ascending=True)
    colors = [ACCENT if rates_s[k] >= 20 else "#78909C" for k in rates_s.index]
    ...
    for i, (k, v) in enumerate(rates_s.items()):
...
def plot_vulnerable_users(df: pd.DataFrame):
    ...
    ax2 = axes[1]
    ped_ru = df[df["ru_label"].isin(["Rural", "Urban"])].groupby("ru_label")["A_PED_bin"].mean() * 100
    mc_ru  = df[df["ru_label"].isin(["Rural", "Urban"])].groupby("ru_label")["A_MC_bin"].mean()  * 100
    lt_ru  = df[df["ru_label"].isin(["Rural", "Urban"])].groupby("ru_label")["A_LT_bin"].mean()  * 100
 
    x = np.arange(2)
    width = 0.25
    ax2.bar(x - width, ped_ru.values, width, label="Pedestrian", color="#E63946")
    ax2.bar(x,         mc_ru.values,  width, label="Motorcycle",  color="#457B9D")
    ax2.bar(x + width, lt_ru.values,  width, label="Large Truck", color="#2D6A4F")
    ax2.set_xticks(x)
...
def plot_driver_age(df: pd.DataFrame):
    ...
    ax = axes[0]
    age_counts = {label: df[col].sum() for col, label in age_cols.items() if col in df.columns}
...
def plot_correlations(df: pd.DataFrame):
    corr_cols = (
        [f"{c}_bin" for c in FLAG_COLS if f"{c}_bin" in df.columns]
        + ["alcohol_involved", "FATALS"]
    )
    corr_labels = {f"{c}_bin": FLAG_COLS[c] for c in FLAG_COLS if f"{c}_bin" in df.columns}
...
def plot_feature_importance(df: pd.DataFrame):
    ...
    feature_labels = {f"{c}_bin": FLAG_COLS[c] for c in FLAG_COLS if f"{c}_bin" in df.columns}
...
def plot_state_crashes(df: pd.DataFrame):
    state_crashes = df.groupby("state_abbr")["ST_CASE"].count().reset_index()
    state_crashes.columns = ["state", "crashes"]
    state_crashes = state_crashes.sort_values("crashes", ascending=False)
 
    fig, ax = plt.subplots(figsize=(15, 5))
    colors = [ACCENT if v >= state_crashes["crashes"].quantile(0.75) else "#90A4AE"
              for v in state_crashes["crashes"]]
...
def run_logistic_regression(df: pd.DataFrame) -> str:
    """
    Model: predict multi-fatality crash (FATALS >= 2).
    Features: impairment/risk binary flags + road/area type indicators.
    """
    feature_cols = (
        [f"{c}_bin" for c in FLAG_COLS if f"{c}_bin" in df.columns]
        + ["alcohol_involved"]
...
def save_summary_stats(df: pd.DataFrame):
    ...
    # Geography
    ru_cnt = df["ru_label"].value_counts().to_dict()
    for k, v in ru_cnt.items():
    ...
    rows.append({"Category": "Impairment", "Metric": "Alcohol Rate (%)",
                 "Value": round(df[df["A_POSBAC"]!=3]["alcohol_involved"].mean()*100, 2)})
    ...
    # Road type
    top_road = df["roadfc_label"].value_counts().idxmax()
    rows.append({"Category": "Road Type", "Metric": "Most Common Road Type", "Value": top_road})
